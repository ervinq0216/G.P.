  ...
  <view class="container">
    ...
    <view class="content-area">
        ...
        <view class="panel">
          ...
          <scroll-view scroll-y class="list-scroll">
            <view class="list-item" v-for="adm in adminList" :key="adm.id">
              <view class="info">
                <text class="name">{{ adm.realName }}</text>
                <text class="sub">账号: {{ adm.username }}</text>
              </view>
      ...
      <view v-if="currentNav === 3" class="module-notice">
        <view class="panel">
          <view class="form-box">
            ...
            <button class="big-add-btn" @click="publishNotice">发布通知</button>
          </view>
          <view class="divider">历史通知</view>
          <scroll-view scroll-y class="list-scroll half">
            <view class="list-item" v-for="n in noticeList" :key="n.id">
              ...
              <text class="sub">{{ n.createdTime }}</text>
            </view>
          </scroll-view>
        </view>
      </view>
      ...
      <view v-if="currentNav === 4" class="module-profile">
        <view class="profile-card">
          ...
          <text class="name">{{ userInfo.realName || '管理员' }}</text>
    ...
    <view class="modal-mask" v-if="showAddAdminModal">
      <view class="modal-content">
        ...
        <input class="modal-input" v-model="adminForm.realName" placeholder="真实姓名" />
...
export default {
  ...
  methods: {
    ...
    fetchNotices() {
      uni.request({ url: 'http://localhost:8080/api/admin/notice/list', success: r => this.noticeList = r.data.data });
    },

    // --- 科室操作 ---
    getDeptsByCategory(cat) {
      return this.deptList.filter(d => d.category === cat);
    },
    openDeptModal(type, category, dept) {
      if (type === 'add') {
        this.deptForm = { id: null, category: category, name: '', intro: '' };
      } else {
        this.deptForm = { ...dept };
      }
    ...
    saveDept() {
      uni.request({
        url: 'http://localhost:8080/api/admin/dept/save',
        method: 'POST',
        data: this.deptForm,
        success: (res) => {
    ...
    deleteDept(id) {
      uni.showModal({
        ...
        success: r => {
          if(r.confirm) {
            uni.request({
              url: `http://localhost:8080/api/admin/dept/delete/${id}`,
              method: 'POST',
              success: () => this.fetchDepts()
            });
          }
        }
      });
    ...
    onDeptChange(e) {
      ...
      this.doctorForm.deptId = this.deptList[idx].id;
      this.selectedDeptName = this.deptList[idx].name;
    },
    saveDoctor() {
      uni.request({
        url: 'http://localhost:8080/api/admin/doctor/add',
        method: 'POST',
        data: this.doctorForm,
        success: (res) => {
          if (res.data.code === 200) {
            this.showAddDoctorModal = false;
            this.doctorForm = { realName: '', jobNumber: '', deptId: null };
    ...
    deleteDoctor(id) {
      uni.showModal({
        ...
        success: r => {
          if(r.confirm) {
            uni.request({ url: `http://localhost:8080/api/admin/doctor/delete/${id}`, method: 'POST', success: () => this.fetchDoctors() });
          }
        }
      });
    },
    getDeptName(id) {
      const d = this.deptList.find(x => x.id === id);
    ...
    deleteAdmin(id) {
      uni.request({ url: `http://localhost:8080/api/admin/delete/${id}`, method: 'POST', success: () => this.fetchAdmins() });
    },

    // --- 请假审批 ---
    auditLeave(id, status) {
      uni.request({
        url: 'http://localhost:8080/api/admin/leave/audit',
        method: 'POST',
        data: { id, status },
        success: () => {
          this.fetchLeaves();
    ...
    // --- 通知发布 ---
    publishNotice() {
      if(!this.noticeForm.title || !this.noticeForm.content) return;
      uni.request({
...
.textarea { border: 1rpx solid #eee; height: 200rpx; padding: 20rpx; border-radius: 8rpx; width: 100%; box-sizing: border-box; }
